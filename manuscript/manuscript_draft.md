...
- **Cross-Species Translational Validation**: Confirmed significant enrichment ($p_{\text{empirical}} = 0.0055$ vs 2,000 size-matched random gene sets) of the vein wall fibroblast activation program in a clinical whole-blood cohort of human VTE patients ($n = 132$).

---

## Abstract

**Background:** Venous thromboembolism (VTE) and post-thrombotic syndrome (PTS) represent a continuum from acute coagulation to chronic vein wall fibrosis. Current target discovery is confounded by data leakage in biomedical knowledge graphs and lacks cellular microenvironment resolution.

**Methods:** We established a leakage-free evaluation benchmark (random stratified 80/10/10 split, 5 independent seeds) for a Tempered Heterogeneous Graph Transformer (Tempered HGT) against classical knowledge graph embeddings across 706 million candidate pairs. GNN-prioritized networks were mapped onto primary single-cell RNA sequencing (21,230 cells) from a mouse inferior vena cava stenosis model (Day 14). Translational conservation was evaluated in a human VTE whole-blood cohort (GSE48000, n=132).

**Results:** Under leakage-free evaluation, Tempered HGT achieved Filtered MRR of 0.086±0.029 and Hits@10 of 0.184±0.068, outperforming the best classical baseline (RotatE) by 2.5-fold. Entity-resolved normalization prioritized SMAD4 as the top-ranked target (Rank #1). Single-cell mapping revealed a lineage division of labor: a myeloid TLR4/NF-κB inflammatory program localized to macrophages, while a TGF-β/SMAD4 fibrotic program partitioned to adventitial fibroblasts. Macrophages were the predominant source of paracrine Tgfb1 ligand (mean=1.05), engaging downstream Smad4 transcription (positive-cell mean=1.19, 41.1% of fibroblasts) and extracellular matrix execution (Col1a1, Fn1) in remodeling fibroblasts. Human whole-blood transcriptomics confirmed significant enrichment of the vein wall fibroblast activation program (NES=1.60, empirical permutation P=0.0055 against 2,000 size-matched random gene sets).

**Conclusion:** Bridging macro-scale graph learning with micro-scale single-cell niche mapping, this study transitions VTE target discovery from liquid-phase coagulation to a macrophage-fibroblast paracrine remodeling program, establishing SMAD4 as a novel therapeutic entry point for post-thrombotic vein wall fibrosis.

**Keywords:** venous thromboembolism, graph neural network, knowledge graph, SMAD4, TGF-beta, single-cell RNA sequencing, post-thrombotic syndrome, target discovery, fibrosis, macrophage-fibroblast crosstalk
...
To evaluate whether the pathological programs identified in the mouse model translate to human disease, we conducted cross-species gene set enrichment analysis (GSEA) on a clinical [21,22] transcriptomic cohort of human VTE whole-blood samples (GSE48000 [18]; $n = 132$, comprising 107 VTE patients and 25 healthy controls) (Figure 5A). Guided by the single-cell mapping (Section 2.3), we derived a vein wall fibroblast activation program comprising 93 human-orthologous genes significantly upregulated in activated vein-wall fibroblasts (a state marked by the protease-activated receptor F2rl1) of the mouse IVC stenosis model, and evaluated this program against the genome-wide expression ranking of human VTE patients versus healthy controls.

The vein wall fibroblast activation program exhibited significant positive enrichment in human VTE whole blood, achieving a Normalized Enrichment Score (NES) of 1.60 (nominal $p = 0.006$) (Figure 5A).

To rule out the possibility that this enrichment signal was driven by isolated individual hub genes, we conducted a systematic Leave-One-Gene-Out (LOGO) sensitivity analysis by iteratively omitting each of the 93 constituent genes from the program definition (Figure 5B). Across all single-gene omissions, the NES remained consistently positive (range: 1.52–1.64) (Figure 5B), confirming that the enrichment reflects a collective, program-level transcriptional signature rather than single-gene bias.

To establish that this enrichment was specific to the program rather than a general property of gene sets of comparable size, we compared the observed enrichment against 2,000 size-matched random gene sets sampled from the human blood transcriptome (Figure 5C). The observed NES (1.60) exceeded 99.5% of the empirical null distribution, yielding an empirical permutation $p$-value of 0.0055 (Figure 5C). Collectively, these findings indicate that the scRNA-defined vein wall fibroblast activation program, representing the cellular microenvironment of the GNN-prioritized SMAD4 axis, is evolutionarily conserved and active in human VTE patients.
...
The significant enrichment of the vein wall fibroblast activation program in human VTE whole-blood transcriptomes (GSE48000, n = 132; NES = 1.60, empirical permutation $p$ = 0.0055), corroborated by leave-one-gene-out robustness [21,22], supports the evolutionary conservation of this pathological program. However, the modest enrichment score underscores an inherent limitation of using whole blood as a surrogate readout for vein wall pathology. While the myeloid TLR4/NF-$\kappa$B inflammatory program represents an acute localized response within the thrombotic vein wall, its whole-blood expression did not reach standalone statistical significance in circulating leukocytes, emphasizing that tissue-restricted stromal programs offer higher translational resolution. Circulating leukocytes capture only a fraction of the tissue-resident fibroblast transcriptional programs that dominate the local vein wall microenvironment. Future validation in human vein wall biopsy specimens or laser-capture microdissected tissue compartments would provide a more direct assessment of clinical translatability.
...
Tempered HGT was benchmarked against four classical Knowledge Graph Embedding (KGE) baselines: TransE, DistMult, ComplEx, and RotatE. All models [7,8,9,10] were trained under identical randomized stratified data splits and evaluated using a Filtered Ranking protocol, wherein known true positive triples in the training set were filtered out during candidate ranking. Ranking metrics included Filtered Mean Reciprocal Rank (Filtered MRR), Hits@10 (proportion of true targets ranked in the top 10 candidates), and Area Under the Receiver Operating Characteristic Curve (AUROC).
...
**Gene Set Enrichment Analysis (GSEA):** A vein wall fibroblast activation program was defined from single-cell differential expression in the mouse IVC model. Differentially expressed genes (DEGs) were identified by comparing F2rl1-positive versus F2rl1-negative activated adventitial fibroblasts in the DVT vein wall (average $\log_2$ fold change > 0.5, adjusted $p < 0.05$); the top-ranked genes were converted to human orthologs and intersected with the human blood transcriptome, yielding a 93-gene signature (F2RL1 itself excluded to avoid single-gene bias). Pre-ranked GSEA was performed using fgsea [22] to calculate Normalized Enrichment Scores (NES) and nominal $p$-values against the genome-wide expression ranking of human VTE patients versus healthy controls.

**Sensitivity and Specificity Controls:** Leave-One-Gene-Out (LOGO) robustness analysis was performed by iteratively re-calculating GSEA after removing each individual gene from the program definition. Empirical permutation control compared enrichment scores against an empirical null distribution generated from 2,000 size-matched random gene sets sampled from the human blood transcriptome, with empirical $p$-values computed using a +1 correction.

### 5. Statistical Analysis

All statistical analyses and visualizations were performed in Python (v3.12; PyTorch Geometric, Scanpy) [27,13] and R (v4.5.2; ggplot2, fgsea, limma, patchwork, Seurat v5.0). Continuous metrics across random seed iterations are presented as Mean $\pm$ Standard Deviation (SD). Nominal $p$-values < 0.05 were considered statistically suggestive. For cross-species validation, empirical permutation $p$-values were computed against 2,000 size-matched random gene sets with a +1 correction.
...
The pre-processed knowledge graph (heterodata.pt, 11 MB), train/validation/test edge splits, and PCA-compressed PubMedBERT node features (features_128d.pt, 41 MB) are publicly available at Zenodo (https://doi.org/10.5281/zenodo.21757258). The mouse scRNA-seq data from the IVC stenosis model are available from the corresponding author upon reasonable request. The human VTE blood transcriptomic dataset GSE48000 is publicly available at the NCBI Gene Expression Omnibus (https://www.ncbi.nlm.nih.gov/geo/). Source data for all figures are provided in the figures/paper_figures/source_data/ directory of the code repository.

All custom code used in this study is publicly available at Zenodo (https://doi.org/10.5281/zenodo.21757258) and on GitHub (https://github.com/JY7123/vte-gnn-target-discovery). The repository includes the complete Tempered HGT implementation, reproduction scripts (reproduce.sh / reproduce.bat), and 124 unit and integration tests. Software dependencies are fully specified in environment.yml. The core computational environment consists of Python 3.12, PyTorch 2.3, and PyTorch Geometric 2.5. Node features were generated using PubMedBERT (microsoft/BiomedNLP-PubMedBERT-base-uncased-abstract-fulltext). Single-cell analysis used Seurat v5 (R) and Scanpy 1.10 (Python). CellChat v2 and dorothea/viper were used for ligand-receptor and transcription factor analyses (R).
...
**Figure 5: Cross-Species Validation of the Vein Wall Fibroblast Activation Program.** (A) GSEA enrichment of the 93-gene vein wall fibroblast activation program in human VTE whole blood (GSE48000, $n = 132$). (B) Leave-One-Gene-Out robustness analysis: NES values after iterative removal of individual constituent genes. (C) Empirical permutation null distribution from 2,000 size-matched random gene sets; the dashed line indicates the observed NES for the vein wall fibroblast activation program.
